...
import { buildMetadataFromYoast, generateArticleJsonLd, generateBreadcrumbJsonLd } from "@/app/lib/seo";
    ...
    if (res.ok) {
      ...
      if (post) {
        return buildMetadataFromYoast(post.yoast_head_json, {
          ...
          publishedTime: post.yoast_head_json?.published_time,
          modifiedTime: post.yoast_head_json?.modified_time,
        });
      }
    }
  } catch {
...
export default async function PostPage({
  ...
  const renderComments = (parentId = 0, depth = 0): JSX.Element[] => {
    ...
    return siblings.map((comment) => (
      <div
        key={comment.id}
        className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-100 animate-fade-in"
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    ...
    { name: "Blog", url: (SITE_BASE || "") + "/" },
    { name: decodeEntities(stripHtml(post.title.rendered)), url: post.link || (SITE_BASE || "") + "/posts/" + slug },
  ]);
